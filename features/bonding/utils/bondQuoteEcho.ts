 ...
 * Buy: wbtcAmountRaw === reviewedInputRaw. Sell: pranaAmountRaw === reviewedInputRaw.
 */
export function isBondingQuoteEchoValid(
  check: BondingQuoteEchoCheck,
): boolean {
  const { quote, mode, termId, reviewedInputRaw } = check;
  ...
  // API amounts are decimal strings; match the locked form/review bigint.
  return echoedInputRaw === reviewedInputRaw.toString();
}

/**
 * Calldata input for create — always the form/review snapshot.
 * Never take the input leg from the quote response (avoids race/regression).
 */
export function resolveCreateAmountRaw(reviewedInputRaw: bigint): bigint {
  return reviewedInputRaw;
}
