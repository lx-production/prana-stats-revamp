...
export const parseOrientationDeg = (orientation: string | null | undefined): [number, number, number] => {
  ...
  if (matches.length >= 3) {
    return matches.slice(0, 3).map((m) => parseFloat(m[1] ?? "0")) as [number, number, number];
  }
  return [0, 0, 0];
};

export const formatOrientationDeg = (x: number, y: number, z: number): string => `${x}deg ${y}deg ${z}deg`;

// Keyboard access: ←/→ rotate
export const createOnKeyDown = (mvRef: RefObject<ModelViewerElement | null>) => {
...
export const createSpinCoin = (
  ...
  return () => {
    ...
    const durationMs = (360 / SPIN_SPEED_DEG_PER_S) * 1000;
