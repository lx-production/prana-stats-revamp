...
export function cacheControlFor(filePath: string): string | null {
  ...
  if (base === 'prana-coin-fallback.png' || base === 'prana-coin.glb') {
    return `public, max-age=${BROWSER_CACHE_TTL_SECONDS.staticAssetsHttp}, immutable`;
  }
  ...
  if (ext === '.json' && base.startsWith('data_')) {
    return `public, max-age=${BROWSER_CACHE_TTL_SECONDS.rootDataJsonHttp}`;
  }
  ...
  if (ext === '.json' && base === 'bonds_v2.json') {
    return `public, max-age=${BROWSER_CACHE_TTL_SECONDS.rootBondsJsonHttp}`;
  }
  ...
  if (ext === '.json' && base === 'buy_dips.json') {
    return `public, max-age=${BROWSER_CACHE_TTL_SECONDS.rootBuyDipsJsonHttp}`;
  }
