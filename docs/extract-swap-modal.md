...
| Chain/contracts dùng chung | `constants/network.ts`, `constants/sharedContracts.ts`, `constants/protocolAddresses.ts`, `types/blockchain.types.ts` | chain ID, public RPC, explorer URLs, PRANA/token addresses, protocol wallets, `Address`/`Hex` | Client lazy features và backend được import trực tiếp; không import ngược từ constants sang feature |
...
- Giữ `types/blockchain.types.ts`, `constants/network.ts`, `constants/sharedContracts.ts` và `constants/protocolAddresses.ts` ở layer trung lập vì frontend và backend cùng dùng. ABI nên tối thiểu và có thể tách theo contract/feature nếu một ABI chỉ có một consumer; không tạo một `web3.ts` tổng hợp re-export tất cả ABI.
