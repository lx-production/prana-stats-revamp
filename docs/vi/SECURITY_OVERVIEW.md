...
- Swap confirmed từ browser được client (`utils/swapTransactionLogs.ts`) chuyển sang `/api/swap/verify-transaction` thay vì endpoint log thường.
...
Chủ yếu nằm trong `hooks/useUniswapQuote.ts` và `hooks/useUniswapSwap.ts`.
...
| Hook UI swap | `hooks/useUniswapQuote.ts`, `hooks/useUniswapSwap.ts`, `utils/swapTransactionLogs.ts` |
