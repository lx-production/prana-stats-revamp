...
On stats homepage load, `utils/prefetchInitialJson.ts` (called from `pages/StatsPage.tsx`) warms `/api/prana-stats` once before hooks mount. The `/stake/` lazy page does not call it. Bond metrics are not prefetched there; see [bond metrics](#computed-api-snapshot-bond-metrics).
...
- Resolved once at Node process start from git (`utils/resolveBuildInfo.ts`): `commit`, `commitShort`, `tag` (exact-match on HEAD or `null`), `branch`, `dirty`, `builtAt`, `repoUrl`.
...
- The Vite client bundle gets the same shape at build time via `define.__APP_BUILD_INFO__` in `config/vite.config.js`; the site footer prefers `tag` (release link) and falls back to the short SHA (commit link).
...
### Top holding addresses API path (v2.1.0 pagination)

- UI list: `hooks/useTopHoldingAddresses.ts` fetches `/api/top-holding-addresses?page=N` (5 holders per page).
...
- `/api/bonding/confirm-transaction` — `private, no-store` (POST; separate confirmation bucket)
...
Markdown document (not JSON). Aggregates live PRANA Protocol stats, bond/staking/capital/LP summaries, performance ranges, FAQ/covenants excerpts, and links to raw JSON endpoints plus `/api/version`. Cached on the server for 1 hour; browsers may reuse the response locally for 30 seconds (`private`).
...
Paginated live balances. Query: `?page=1` (default) or `?page=2` for the current 10-address list (`TOP_HOLDERS_PAGE_SIZE = 5`). Invalid/out-of-range `page` falls back to `1`.
...
```json
{
  ...
  "rpcUrl": "https://…",
  "token": {
    "address": "0x928277e774f34272717eadfafc3fd802dafbd0f5",
    "symbol": "PRANA",
    "decimals": 9
  },
  "holders": [
...
`holders.length` is at most one page (5). The React hook does not re-slice this array for compatibility with older full-list responses.
